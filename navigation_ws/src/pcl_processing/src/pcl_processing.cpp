...
void getcloud_vec(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg) {
    ...
    if((pcl2cloud->points.size()) == 0){
        ...
        long point_num = 0;
        float min_z_threshold = 0.1; // minimum height threshold to filter out ground points
        ...
        for (long i = 0; i <= pcl2cloud->points.size(); i = i + 1) {
            ...
            if (gradient > 1.0f) {
                ...
                // only consider points outside of robot body (0.3m radius)
                // TODO: change according to robot size
                if(pow(pcl2cloud->points[i].x - current_x, 2) + pow(pcl2cloud->points[i].y - current_y, 2) > 0.09){
                    // only consider points within a circle (2m radius)
                    if(pow(pcl2cloud->points[i].x - current_x, 2) + pow(pcl2cloud->points[i].y - current_y, 2) < 4){
                        if (pcl2cloud->points[i].z >= min_z_threshold){
